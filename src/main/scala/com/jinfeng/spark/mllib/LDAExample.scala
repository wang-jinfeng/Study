...
object LDAExample {
  ...
  private def run(params: Params) {
    ...
    val optimizer = params.algorithm.toLowerCase match {
      ...
      //        add (1.0/actualCorpusSize) to MiniBatchFraction be more robust on tiny dataset
    ...
    if (ldaModel.isInstanceOf[DistributedLDAModel]) {
      ...
      //      val avgLogLikeihood = distLDAModel.logLikelihood / actualCorpusSize.toDouble
      //      println(s"\t Training Data average log likelihood:$avgLogLikeihood")
      //      println()
    }
  ...
  private def preprocess(sc: SparkContext,
    ...
    // Get dataset document texts
    // One document per line in each text file.If the input consists of may small files,this can result in a
    // large number of small partitions,which can degrade performance.
